...
class SetFunctionTaylorLastLinear(object):
    ...
    def _compute_per_element_grads(self, theta_init,x_trn,y_trn):
        ...
        self.embDim = self.model.get_feature_dim()
        #embedding = torch.zeros([len(Y),  embDim*self.num_classes],device=self.device)
        
        with torch.no_grad():
            scores,feat = self.model(inputs,True)
            data = F.softmax(scores, dim=1)
                
        l = len(targets)
        ind = torch.arange(0,self.embDim,device=self.device).repeat(l,1)
        max_ind = ind + (targets*self.embDim)[:,None]

        soft_max = -1*torch.repeat_interleave(data, feat.shape[1], dim=1) 
        embedding = (feat.repeat(1,self.num_classes) * soft_max).scatter_add_(1, max_ind, feat)
        '''tmp_tensor = torch.zeros(len(inputs), self.num_classes).to(self.device)
        tmp_tensor.scatter_(1, targets.view(-1,1), 1)
        outputs = tmp_tensor
    ...
    def _update_grads_val(self, theta_init, grads_currX=None, first_init=False):
        ...
        if first_init:
            with torch.no_grad():
                scores,feat = self.model(self.x_val,True)
                data = F.softmax(scores, dim=1)
                        
            ind = torch.arange(0,self.embDim,device=self.device).repeat(l,1)
            max_ind = ind + (self.y_val*self.embDim)[:,None]

            soft_max = -1*torch.repeat_interleave(data, feat.shape[1], dim=1) 
            grads = (feat.repeat(1,self.num_classes) * soft_max).scatter_add_(1, max_ind, feat)
        ...
        # populate the gradients in model params based on loss.
            ...
            with torch.no_grad():
                ...
                '''for p in self.model.parameters():
                    if p.requires_grad:
                         print(p.name,len(p.data))'''
                params[-2].view(-1).data.sub_(self.eta * grads_currX)
                scores,feat = self.model(self.x_val,True)
                data = F.softmax(scores, dim=1)
    ...
    def eval_taylor_modular(self, grads, theta_init):
        ...
        with torch.no_grad():
            #grads_tensor = torch.cat(grads, dim=0)
            param_update = self.eta * grads#_tensor
            gains = torch.matmul(param_update, grads_val)
        return gains
    ...
    def naive_greedy_max(self, x_trn, y_trn, budget, theta_init,previous=None,random=False):
        ...
        while (numSelected < budget):
            ...
            rem_grads = [self.grads_per_elem[x].view(1, self.grads_per_elem[0].shape[0]) for x in subset_selected]
            gains = self.eval_taylor_modular(rem_grads, theta_init)'''
            gains = self.eval_taylor_modular(self.grads_per_elem[remainSet], theta_init)
            # Update the greedy set and remaining set
            bestId = remainSet[torch.argmax(gains)]#subset_selected[torch.argmax(gains)]
            greedySet.append(bestId)
            remainSet.remove(bestId)
            # Update info in grads_currX using element=bestId
            if numSelected > 0:
