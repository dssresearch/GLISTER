...
class SetFunctionLoader_2(object):
    ...
    def _compute_per_element_grads(self, theta_init):
        ...
        for batch_idx in batch_wise_indices:
            inputs = torch.cat([self.trainset[x][0].view(-1, 1, self.trainset[x][0].shape[1], self.trainset[x][0].shape[2]) for x in batch_idx], dim=0).type(torch.float)
...
class PriorSetFunctionLoader_2(object):
    ...
    def naive_greedy_max(self, budget, theta_init):
        ...
        for i in range(self.num_classes):
            ...
            numSelected = 0
            greedySet = list()
            remainSet = [x.item() for x in torch.where(self.trainset.dataset.targets[self.trainset.indices] == i)[0]]
...
class SetFunctionSupervisedCRAIG(object):
    ...
    def compute_score(self, idxs):
        ...
        batch_wise_indices = actual_idxs[list(BatchSampler(SequentialSampler(actual_idxs),self.trn_batch_size, drop_last=False))]
        cnt = 0
        ...
        for i in range(math.ceil(len(vector)/self.trn_batch_size)):
            ...
            for j in range(math.ceil(len(vector)/self.trn_batch_size)):
                ...
                self.dist_mat[i*self.trn_batch_size: (i+1)*self.trn_batch_size ,j*self.trn_batch_size: (j+1)*self.trn_batch_size] \
