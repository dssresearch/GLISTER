...
from models.set_function_grad_computation_taylor import GlisterSetFunction_Closed as SetFunction
from models.set_function_grad_computation_taylor import GLISTER_WeightedSetFunction as WtSetFunction
import math
...
def train_model_craig(start_rand_idxs, bud):
    ...
    for i in range(0, num_epochs):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (inputs, targets) in enumerate(trainloader):
                ...
                trn_accu = 100 * trn_correct / trn_total
    ...
    for i in range(num_epochs):
        timing = timing + "," + str(time_array[i])
    ...
    print("SelectionRun---------------------------------")
    print("Final SubsetTrn and FullTrn Loss:", substrn_losses[-1], fulltrn_losses[-1])
    print("Validation Loss and Accuracy:", val_loss, val_accu)
    print("Test Data Loss and Accuracy:", tst_loss , tst_accu)
    print('-----------------------------------')
    return val_accu, tst_accu, subtrn_accu, trn_accu, val_loss, tst_loss, subtrn_loss, full_trn_loss, val_losses, substrn_losses, fulltrn_losses, idxs, np.sum(time_array)
...
def random_greedy_train_model_online_taylor(start_rand_idxs, bud, lam):
    ...
    for i in range(0, num_epochs):
        ...
        if i % print_every == 0:  # Print Training and Validation Loss
            print('Epoch:', i + 1, 'SubsetTrn,FullTrn,ValLoss:', subtrn_loss, full_trn_loss, val_loss)
        # Waits for everything to finish running
    print("Random Greedy", file=logfile)
    print('---------------------------------------------------------------------', file=logfile)
    val = "Validation Accuracy,"
    tst = "Test Accuracy,"
    timing = "Time,"
...
def facloc_reg_train_model_online_taylor(start_rand_idxs, facloc_idxs, bud, lam):
    ...
    for i in range(0, num_epochs):
        ...
        if i % print_every == 0:  # Print Training and Validation Loss
            print('Epoch:', i + 1, 'SubsetTrn,FullTrn,ValLoss:', subtrn_loss, full_trn_loss, val_loss)
        # Waits for everything to finish running
    print("Facloc GLISTER", file=logfile)
    print('---------------------------------------------------------------------', file=logfile)
    val = "Validation Accuracy,"
    tst = "Test Accuracy,"
    timing = "Time,"
...
def train_model_mod_online(start_rand_idxs, bud):
    ...
    tst_acc = np.zeros(num_epochs)
    # cosine learning rate
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, len(trainloader) * num_epochs)
    ...
    print("Validation Loss and Accuracy:", val_loss, val_accu)
    print("Test Data Loss and Accuracy:", test_loss, tst_accu)
    print('-----------------------------------')
    return val_acc, tst_acc,  subtrn_acc, full_trn_acc, val_loss, test_loss, subtrn_loss, full_trn_loss, val_losses, substrn_losses, fulltrn_losses, idxs, np.sum(time_array)
...
def train_model_random(start_rand_idxs):
    ...
    for i in range(0, num_epochs):
        ...
        print("Random training", file=logfile)
        print('---------------------------------------------------------------------', file=logfile)
        val = "Validation Accuracy,"
        tst = "Test Accuracy,"
        timing = "Time,"
    ...
    print("Test Data Loss and Accuracy:", test_loss, tst_accu)
    print('-----------------------------------')
    return val_acc, tst_acc,  subtrn_acc, full_trn_acc, val_loss, test_loss, subtrn_loss, full_trn_loss, val_losses, substrn_losses, fulltrn_losses, np.sum(time_array)
