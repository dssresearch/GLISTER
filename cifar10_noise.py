...
from models.cifar_set_function_grad_computation_taylor import SetFunctionLoader_2 as SetFunction
from models.cifar_set_function_grad_computation_taylor import WeightedSetFunctionLoader as WtSetFunction
import math
from models.cifar10net import CifarNet
...
noise_ratio = float(sys.argv[6])
warm_method = 0  # whether to use warmstart-onestep (1) or online (0)
num_runs = 1  # number of random runs
learning_rate = 0.05
all_logs_dir = './results/debugging/' + data_name  + '_noise/' + str(fraction) + '/' + str(select_every)
print(all_logs_dir)
subprocess.run(["mkdir", all_logs_dir])
path_logfile = os.path.join(all_logs_dir, data_name + '_' + str(noise_ratio) + '.txt')
...
if data_name == 'cifar10':
    ...
    val_batch_size = 1000
    tst_batch_size = 1000
    noise_bud = int(len(trainset.indices) * noise_ratio)
    noise_indices = np.random.choice(trainset.indices, size=noise_bud, replace=False)
    trainset.noise_indices = noise_indices
    trainset.original_labels = [trainset.dataset.targets[x] for x in noise_indices]
    noisy_labels = np.random.choice(np.arange(num_cls), size=noise_bud, replace=True)
    for x in range(len(noise_indices)):
        trainset.dataset.targets[noise_indices[x]] = noisy_labels[x]
...
one_step_noise_selected = (1.0 * len(list(set(one_subset_idxs).intersection(set(trainset.noise_indices)))))/bud
...
facloc_reg_noise_selected = (1.0 * len(list(set(facloc_reg_subset_idxs).intersection(set(trainset.noise_indices)))))/bud
...
rand_reg_noise_selected = (1.0 * len(list(set(rand_reg_subset_idxs).intersection(set(trainset.noise_indices)))))/bud
...
plt_file = path_logfile + '_' + str(fraction) +'_' + str(noise_ratio) + 'substrn_loss_v=VAL.png'
...
plt_file = path_logfile + '_' + str(fraction) + '_' + str(noise_ratio) +'fulltrn_loss_v=VAL.png'
plt.savefig(plt_file)
plt.clf()
...
plt_file = path_logfile + '_' + str(fraction) + '_' + str(noise_ratio) +'valloss_v=VAL.png'
plt.savefig(plt_file)
plt.clf()
...
print('*| facloc regularizec Taylor after training               |', facloc_one_step_time, '  | ',
       file=logfile)
print('---------------------------------------------------', file=logfile)
print('|Algo                            | Noisy Labels Ratio       |', file=logfile)
print('| -------------------------------|:-------------:|', file=logfile)
print('*| Taylor with Validation=VAL     |', one_step_noise_selected, '  | ', file=logfile)
print('*| random regularized Taylor after training               |', rand_reg_noise_selected,' |',
      file=logfile)
print('*| facloc regularizec Taylor after training               |', facloc_reg_noise_selected, '  | ',
       file=logfile)
print("\n", file=logfile)
...
with open(all_logs_dir + '/mod_one_step_subset_selected'+ '_' + str(noise_ratio) +'.txt', 'w') as log_file:
    print(mod_subset_idxs, file=log_file)

subset_idxs = list(one_subset_idxs)
with open(all_logs_dir + '/one_step_subset_selected'+ '_' + str(noise_ratio) +'.txt', 'w') as log_file:
    print(subset_idxs, file=log_file)

rand_subset_idxs = list(rand_reg_subset_idxs)
with open(all_logs_dir + '/rand_reg_one_step_subset_selected'+ '_' + str(noise_ratio) +'.txt', 'w') as log_file:
    print(rand_subset_idxs, file=log_file)

facloc_reg_subset_idxs = list(facloc_reg_subset_idxs)
with open(all_logs_dir + '/facloc_reg_one_step_subset_selected'+ '_' + str(noise_ratio) +'.txt', 'w') as log_file:
    print(facloc_reg_subset_idxs, file=log_file)

random_subset_idx = list(random_subset_idx)
with open(all_logs_dir + '/random_subset_selected'+ '_' + str(noise_ratio) + '.txt', 'w') as log_file:
    print(random_subset_idx, file=log_file)

facloc_idxs = list(facloc_idxs)
with open(all_logs_dir + '/facloc_subset_selected'+ '_' + str(noise_ratio) +'.txt', 'w') as log_file:
