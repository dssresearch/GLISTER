...
from models.new_cifar10_set_function import SetFunctionLoader_2 as SetFunction
from models.new_cifar10_set_function import WeightedSetFunctionLoader as WtSetFunction
from models.new_cifar10_set_function import SetFunctionSupervisedCRAIG as CRAIG
import math
from models.cifar10net1 import CifarNet
...
learning_rate = 0.05
all_logs_dir = './results/debugging/' + data_name + '_new_grad/' + str(fraction) + '/' + str(select_every)
print(all_logs_dir)
...
def train_CRAIG(bud):
    ...
    for i in range(0, num_epochs):
        ...
        for batch_idx in batch_wise_indices:
            inputs = torch.cat(
                ...
                dim=0).type(torch.float)
            targets = torch.tensor([fullset[x][1] for x in batch_idx])
            gamma = torch.tensor([Gamma[(actual_idxs == x).nonzero()] for x in batch_idx]).to(device)

            inputs, targets = inputs.to(device), targets.to(device,
                                                            non_blocking=True)  # targets can have non_blocking=True.
            optimizer.zero_grad()
            outputs, _ = model(inputs)
            loss = criterion(outputs * gamma, targets)
    ...
    print('-----------------------------------')
    return val_acc, tst_acc, full_trn_acc, val_loss, test_loss, full_trn_loss, val_losses, substrn_losses, fulltrn_losses, idxs, time
...
"""
# CRAIG algo run
craig_valacc, craig_tstacc, craig_full_trn_acc, craig_valloss, craig_tstloss, craig_full_trn_loss, craig_val_losses, \
craig_substrn_losses, craig_fulltrn_losses, craig_subset_idxs, craig_step_time = train_CRAIG(bud)
"""
...
#plt.plot(np.arange(plot_start_epoch, num_epochs), craig_substrn_losses[plot_start_epoch:], '#750D86', label='CRAIG')
plt.plot(np.arange(plot_start_epoch, num_epochs), rand_substrn_losses[plot_start_epoch:], 'g-', label='random')
plt.plot(np.arange(plot_start_epoch, num_epochs), ol_rand_substrn_losses[plot_start_epoch:], 'g+',
         label='Online random')
...
plt.plot(np.arange(plot_start_epoch, num_epochs), ol_rand_fulltrn_losses[plot_start_epoch:], 'g+',
         label='Online random')
...
print('| -------------------------------|:-------------:| ----------------:|', file=logfile)
print('*| Facility Location             |', facloc_valacc * 100, '  | ', facloc_tstacc * 100, ' |', file=logfile)
#print('*| CRAIG     |', craig_valacc * 100, '  | ', craig_tstacc * 100, ' |', file=logfile)
print('*| Taylor with Validation=VAL     |', one_val_valacc * 100, '  | ', one_val_tstacc * 100, ' |', file=logfile)
print('*| Random Selection               |', rand_valacc * 100, '  | ', rand_tstacc * 100, ' |', file=logfile)
print('*| Online Random Selection               |', ol_rand_valacc * 100, '  | ', ol_rand_tstacc * 100, ' |',
      file=logfile)
print('*| Taylor after training               |', mod_val_valacc * 100, '  | ', mod_val_tstacc * 100, ' |',
      file=logfile)
print('*| random regularized Taylor after training               |', rand_reg_val_valacc * 100, '  | ',
      rand_reg_val_tstacc * 100, ' |',
      file=logfile)
print('*| facloc regularizec Taylor after training               |', facloc_reg_val_valacc * 100, '  | ',
      facloc_reg_val_tstacc * 100, ' |', file=logfile)
print('---------------------------------------------------', file=logfile)
print('|Algo                            | Run Time       |', file=logfile)
print('| -------------------------------|:-------------:|', file=logfile)
print('*| Facility Location             |', facloc_time, '  | ', file=logfile)
#print('*| CRAIG     |', craig_step_time, '  | ', file=logfile)
...
"""
subset_idxs = list(craig_subset_idxs)
