...
from models.set_function_all import SetFunctionFacLoc, SetFunctionBatch
from models.set_function_all import SetFunctionTaylorDeep_Super
from models.simpleNN_net import *
# from resnet import *
# from models.mnist_net import MnistNet
from utils.custom_dataset import *
from utils.custom_dataset import write_knndata
from visualize import describe_data
...
all_logs_dir = './results/Super/' + data_name
print(all_logs_dir)
...
x_trn = torch.from_numpy(x_trn.astype(np.float32)).to(device)
x_val = torch.from_numpy(x_val.astype(np.float32)).to(device)
x_tst = torch.from_numpy(x_tst.astype(np.float32)).to(device)
y_trn = torch.from_numpy(y_trn.astype(np.int64)).to(device)
y_val = torch.from_numpy(y_val.astype(np.int64)).to(device)
y_tst = torch.from_numpy(y_tst.astype(np.int64)).to(device)
...
def train_model(set_function,start_rand_idxs, bud=None, valid=None):
    ...
    if set_function == "Facility Location":
    ...
    if fraction != 1:
        ...
        if set_function not in ["TaylorDeep","Online-Random","OneStep","TaylorDeepSuper"]:
            describe_data(x_trn_sub, y_trn_sub,num_cls,detail[-2]+"_"+detail[-1]+"_",path, open(path+"/"+data_name+".txt", 'w') )
    ...
    for i in range(num_epochs):    
        ...
        if ((i+1) % select_every == 0 and fraction != 1):
            ...
            if set_function == "TaylorDeep" or set_function == "OneStep" :
                ...
                subprocess.run(["mkdir", path+"/time_"+str(int((i+1) / select_every))])
                describe_data(x_trn_sub, y_trn_sub,num_cls,detail[-2]+"_"+detail[-1]+"_",path+"/time_"+str(int((i+1) / select_every)), 
                    open(path+"/time_"+str(int((i+1) / select_every))+"/"+data_name+".txt", 'w') )
                for item in range(math.ceil(len(x_trn_sub)/train_batch_size)):
                  inputs = x_trn_sub[item*train_batch_size:(item+1)*train_batch_size]
                  target  = y_trn_sub[item*train_batch_size:(item+1)*train_batch_size]
                  train_sub_loader.append((inputs,target))
                ...
                #train_sub_loader = torch.utils.data.DataLoader(torch.utils.data.Subset(trainset, idxs),batch_size=train_batch_size, 
                    #shuffle=False, **kwargs)#num_workers=2) #torch.utils.data.Subset(trainset, idxs)
                ...
                #print("selEpoch: %d, Starting Selection:" % i, str(datetime.datetime.now()),file=logfile)
                t_ng_start = time.time()
                new_idxs = setf_model.class_wise(bud, clone_dict) #, grads_idxs
                idxs = new_idxs     # update the current set
...
subprocess.run(["mkdir", 'data_description/'+data_name])
subprocess.run(["mkdir", 'data_description/'+data_name+"/method"])
subprocess.run(["mkdir", 'data_description/'+data_name+"/method/fc"])
subprocess.run(["mkdir", 'data_description/'+data_name+"/method/knn_trn"])
subprocess.run(["mkdir", 'data_description/'+data_name+"/method/knn_val"])
subprocess.run(["mkdir", 'data_description/'+data_name+"/method/onestep_trn"])
subprocess.run(["mkdir", 'data_description/'+data_name+"/method/onestep_val"])
subprocess.run(["mkdir", 'data_description/'+data_name+"/method/onestep_taylor_trn"])
subprocess.run(["mkdir", 'data_description/'+data_name+"/method/onestep_taylor_val"])
subprocess.run(["mkdir", 'data_description/'+data_name+"/method/random"])
subprocess.run(["mkdir", 'data_description/'+data_name+"/method/on_random"])
...
for fraction in list_fractions:
    ...
    path = 'data_description/'+data_name+"/method/fc/"+str(fraction)
    subprocess.run(["mkdir", path])
    fc_valacc, fc_tstacc, fc_valloss, fc_tstloss, fc_substrn_losses, fc_fulltrn_losses, fc_val_losses= train_model("Facility Location",None,bud)
    ...
    path = 'data_description/'+data_name+"/method/knn_trn/"+str(fraction)
    subprocess.run(["mkdir", path])
    ...
    path = 'data_description/'+data_name+"/method/knn_val/"+str(fraction)
    subprocess.run(["mkdir", path])
    ...
    # OneStep Taylor Runs    
    path = 'data_description/'+data_name+"/method/onestep_taylor_trn/"+str(fraction)
    subprocess.run(["mkdir", path])
    t_trn_valacc, t_trn_tstacc, t_trn_valloss, t_trn_tstloss, tay_ftrn_substrn_losses, tay_ftrn_fulltrn_losses, tay_ftrn_val_losses = train_model("TaylorDeepSuper",start_idxs,bud,False)

    path = 'data_description/'+data_name+"/method/onestep_taylor_val/"+str(fraction)
    subprocess.run(["mkdir", path])
    t_val_valacc, t_val_tstacc, t_val_valloss, t_val_tstloss, tay_fval_substrn_losses, tay_fval_fulltrn_losses, tay_fval_val_losses = train_model("TaylorDeepSuper",start_idxs,bud,True)
    
    
    #Random Run
    path = 'data_description/'+data_name+"/method/random/"+str(fraction)
    subprocess.run(["mkdir", path]) 
    ...
    path = 'data_description/'+data_name+"/method/on_random/"+str(fraction)
    subprocess.run(["mkdir", path]) 
    ...
    '''path = 'data_description/'+data_name+"/method/onestep_trn/"+str(fraction)
    subprocess.run(["mkdir", path])   
    nt_trn_valacc, nt_trn_tstacc, nt_trn_valloss, nt_trn_tstloss, nontay_ftrn_substrn_losses, nontay_ftrn_fulltrn_losses, nontay_ftrn_val_losses = train_model("OneStep",start_idxs,bud,False)#, do_compare=compare_method)

    path = 'data_description/'+data_name+"/method/onestep_val/"+str(fraction)
    subprocess.run(["mkdir", path]) 
    nt_val_valacc, nt_val_tstacc, nt_val_valloss, nt_val_tstloss, nontay_fval_substrn_losses, nontay_fval_fulltrn_losses, nontay_fval_val_losses = train_model("OneStep",start_idxs,bud,True)#, do_compare=compare_method)
    '''
