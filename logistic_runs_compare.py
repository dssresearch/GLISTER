...
from models.set_function_all import SetFunctionTaylor
...
all_logs_dir = './results/' + data_name
print(all_logs_dir)
...
def train_model_online_old(start_rand_idxs, bud, valid, do_compare):
    torch.manual_seed(42)
    np.random.seed(42)
    ...
    # Just return dummy zero values 
    if not do_compare:
        return (0, 0, 0, 0, substrn_losses, fulltrn_losses, val_losses)
    ...
    print("Starting Online OneStep Run with NON-taylor method!")
    # if valid:
    #     print("Online OneStep Run with Taylor approximation and with Validation Set",file=logfile)
    # else:
    #     print("Online OneStep Run with Taylor approximation and without Validation Set",file=logfile)
...
for fraction in list_fractions:
    ...
    knn_valacc_flagtrn, knn_tstacc_flagtrn, knn_valloss_flagtrn, knn_tstloss_flagtrn, knn_ftrn_substrn_losses, knn_ftrn_fulltrn_losses, knn_ftrn_val_losses = train_model_knnsb(knn_idxs_flag_trn)
    knn_valacc_flagval, knn_tstacc_flagval, knn_valloss_flagval, knn_tstloss_flagval, knn_fval_substrn_losses, knn_fval_fulltrn_losses, knn_fval_val_losses = train_model_knnsb(knn_idxs_flag_val)
    ...
    orv1, ort1, orv2, ort2, o_rand_substrn_losses, o_rand_fulltrn_losses, o_rand_val_losses = train_model_online_random(start_idxs, bud)    
    ...
    print('*| Random Selection               |', orv1,              '  | ', ort1,              ' |',file=logfile)
