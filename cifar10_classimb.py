...
import math
from models.resnet import ResNet18 as CifarNet
...
def train_model_craig(start_rand_idxs, bud):
    ...
    for i in range(0, num_epochs):
        ...
        for batch_idx in batch_wise_indices:
            inputs = torch.cat(
                [fullset[actual_idxs[x]][0].view(-1, 3, fullset[actual_idxs[x]][0].shape[1], fullset[actual_idxs[x]][0].shape[2]) for x in batch_idx],
                dim=0).type(torch.float)
...
random_prior_subset_idx = [trainset.indices[x] for x in rand_prior_idxs]
...
# Random Prior Run
rand_prior_valacc, rand_prior_tstacc, rand_prior_subtrn_acc, rand_prior_full_trn_acc, rand_prior_valloss, rand_prior_tstloss, rand_prior_subtrnloss,\
rand_prior_full_trn_loss, rand_prior_val_losses, rand_prior_substrn_losses, rand_prior_fulltrn_losses, \
random_prior_run_time = train_model_random(rand_prior_idxs)
print("Random Prior Run Time: ", random_prior_run_time)
...
plt.plot(np.arange(plot_start_epoch, num_epochs), rand_prior_substrn_losses[plot_start_epoch:], '#750D86', label='random with prior')
...
plt.plot(np.arange(plot_start_epoch, num_epochs), rand_prior_fulltrn_losses[plot_start_epoch:], '#750D86', label='random with prior')
...
plt.plot(np.arange(plot_start_epoch, num_epochs), rand_prior_val_losses[plot_start_epoch:], '#750D86', label='random with prior')
...
print('*| Random with Fixed Prior Selection               |', rand_prior_valacc, '  | ', rand_prior_tstacc, ' |', file=logfile)
...
random_prior_subset_idx = list(random_prior_subset_idx)
with open(all_logs_dir + '/random_prior_subset_selected.txt', 'w') as log_file:
    print(random_prior_subset_idx, file=log_file)
"""
